...
import net.link.util.util.ObjectMeta.For;
import net.link.util.util.TypeUtils.LastResult;
...
public abstract class ObjectUtils {
    ...
    public static String describe(final Object o) {
        ...
        if (byte[].class.isInstance( o )) {
            ...
            // Decode some bytes.
            CharBuffer decodedBytes = Charsets.UTF_8.decode( ByteBuffer.wrap( byteArray, 0, Math.min( byteArray.length, MAX_DECODE_LENGTH ) ) );
    ...
    public static String toString(final Object o) {
        ...
        try {
            toString.append( forEachFieldWithMeta( For.toString, o.getClass(), new NFunctionNN<LastResult<Field, StringBuilder>, StringBuilder>() {
                @Override
                @SuppressWarnings("ParameterNameDiffersFromOverriddenParameter")
                public StringBuilder apply(@Nonnull final TypeUtils.LastResult<Field, StringBuilder> lastResult) {
    ...
    public static int hashCode(final Object o) {
        ...
        if (seen.get( For.hashCode ).get().contains( identityHashCode )) {
            // Cyclic reference.
            logger.trc( "%s- Detected cycle, returning identity.", StringUtils.indent( seen.get( For.hashCode ).get().size() + 1 ), identityHashCode );
    ...
    public static boolean equals(@Nullable final Object superObject, @Nullable final Object subObject) {
        ...
        try {
            ...
            return ifNotNullElse( forEachFieldWithMeta( For.equals, superObject.getClass(), new NFunctionNN<LastResult<Field, Boolean>, Boolean>() {
                @Nonnull
                @Override
                @SuppressWarnings("ParameterNameDiffersFromOverriddenParameter")
    ...
    private static <T> boolean usesMeta(final For meta, final Class<T> type) {
        ...
        for (final Map.Entry<Class<? super T>, Map<Class<?>, ObjectMeta>> annotationEntry : TypeUtils.getAnnotations( type, ObjectMeta.class ).entrySet()) {
     ...
     */
    @Nonnull
    public static <F, T> T ifNotNullElse(@Nullable final F from, final NNFunctionNN<F, T> notNullValueFunction, @Nonnull final T nullValue) {
     ...
     */
    @Nullable
    public static <F, T> T ifNotNullElseNullable(@Nullable final F from, final NFunctionNN<F, T> notNullValueFunction, @Nullable final T nullValue) {
